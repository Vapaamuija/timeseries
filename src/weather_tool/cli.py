...
from .utils.helpers import validate_icao_code, parse_time_range
...
def main(ctx, config, log_level, log_file):
    ...
    if config:
        ...
        if default_config_path.exists():
            ctx.obj = Config.from_file(str(default_config_path))
...
@click.option('--start-time', help='Start time (YYYY-MM-DD HH:MM or relative like "24 hours ago")')
@click.option('--end-time', help='End time (YYYY-MM-DD HH:MM or relative like "now")')
@click.option('--time-range', help='Time range (e.g., "next 6 hours", "last 24 hours", "next 3 days")')
@click.option('--variables', help='Comma-separated list of variables to plot')
@click.option('--data-source', default='auto', type=click.Choice(['auto', 'metno', 'http', 'file']),
              help='Data source preference')
@click.option('--symbols', type=click.Choice(['ascii', 'unicode', 'svg']),
              help='Weather symbol type')
@click.option('--output', '-o', help='Output file path')
@click.option('--show/--no-show', default=False, help='Display plot after creation')
@click.pass_obj
def plot(config, icao_code, start_time, end_time, time_range, variables, data_source, symbols, output, show):
    ...
    try:
        ...
        if time_range:
            ...
            if start_dt is None or end_dt is None:
                ...
                click.echo("Examples: 'next 6 hours', 'last 24 hours', 'next 3 days'", err=True)
                return
        else:
        ...
        # Create plotter
        plotter = WeatherPlotter(config)
        
        click.echo(f"Plotting weather data for {icao_code}...")
        
        # Create plot with new options (always Norwegian meteogram style)
        ...
        logging.error(f"Plot command failed: {e}")
...
@click.option('--time-range', help='Time range (e.g., "next 6 hours", "last 24 hours", "next 3 days")')
@click.option('--variables', help='Comma-separated list of variables to plot')
@click.option('--data-source', default='auto', type=click.Choice(['auto', 'api', 'files']),
              help='Data source preference')
...
@click.pass_obj
def plot_multiple(config, icao_codes, start_time, end_time, time_range, variables, data_source, output_dir, show_symbols):
    ...
    try:
        ...
        if invalid_codes:
            click.echo(f"Error: Invalid ICAO codes: {', '.join(invalid_codes)}", err=True)
            return
        ...
        click.echo(f"Error: {e}", err=True)
        logging.error(f"Plot multiple command failed: {e}")
...
@click.pass_obj
def list_airports(config, country, airport_type, limit):
    ...
    try:
        ...
        click.echo(f"Error: {e}", err=True)
        logging.error(f"List airports command failed: {e}")
...
@click.pass_obj
def test_connection(config, api, files):
    ...
    try:
        ...
        if api:
            ...
            try:
                if plotter.api_client.test_connection():
        ...
        click.echo(f"Error: {e}", err=True)
        logging.error(f"Test connection command failed: {e}")
...
def export_config(config, output):
    ...
    try:
        ...
        click.echo(f"Error: {e}", err=True)
        logging.error(f"Export config command failed: {e}")
