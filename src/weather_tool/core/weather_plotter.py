...
from ..data.interfaces import WeatherDataClient, DataClientRegistry
from ..data.metno_unified import MetNoUnifiedClient, MetNoHTTPClient, MetNoFileClient
from ..airports.manager import AirportManager
from ..plotting.interfaces import PlotConfig, SymbolType
...
class WeatherPlotter:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.metno_client = MetNoUnifiedClient(self.config)      # Primary: HTTP + File combined
        ...
        symbol_type = SymbolType.SVG  # Default (only supported type)
        ...
        # Only Norwegian meteogram plotter is supported
        self.norwegian_plotter = MeteogramPlotter(plot_config)
    ...
    def _setup_matplotlib(self) -> None:
        ...
        # Set style with explicit layout control
        try:
        ...
        plt.rcParams.update({
            ...
            'figure.autolayout': False,  # Disable automatic layout to prevent warnings
            'figure.constrained_layout.use': False,  # Also disable constrained layout
        })
        
        # Apply additional font settings if available
    ...
    def plot_airport(
        ...
        # Create Norwegian meteogram plot
        # Create custom plotter if symbol type is specified
        if symbol_type:
            from ..plotting.interfaces import SymbolType as PlotSymbolType
            ...
            # Use default Norwegian meteogram plotter
            fig = self.norwegian_plotter.create_plot(weather_data, airport)
    ...
    def _get_data_client(self, data_source: str) -> WeatherDataClient:
        ...
        if data_source == "auto" or data_source == "metno":
            ...
            if client:
                ...
                raise ValueError(f"Invalid data source: {data_source}. Available: auto, metno, http, file")
    ...
    def _save_plot(self, fig: Figure, output_path: str) -> None:
        ...
        try:
            ...
            with plt.rc_context({'figure.autolayout': False}):
                # Explicitly disable tight layout before saving
            ...
            fig.savefig(output_path, dpi=72, facecolor='white')
