...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        api_user_agent = os.getenv("WEATHER_API_USER_AGENT")
        if api_user_agent:
            config.api.user_agent = api_user_agent

        # THREDDS configuration
        thredds_base_url = os.getenv("WEATHER_THREDDS_BASE_URL")
        if thredds_base_url:
            config.thredds.base_url = thredds_base_url
        thredds_data_path = os.getenv("WEATHER_THREDDS_DATA_PATH")
        if thredds_data_path:
            config.thredds.data_path = thredds_data_path

        # Directories
        data_dir = os.getenv("WEATHER_DATA_DIR")
        if data_dir:
            config.data_dir = Path(data_dir)
        output_dir = os.getenv("WEATHER_OUTPUT_DIR")
        if output_dir:
