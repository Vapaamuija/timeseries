...
class WeatherDataClient(ABC):
    ...
    def get_available_variables(self) -> List[str]:
        ...
        return [
            ...
            'dew_point', 'uv_index', 'cloud_high', 'cloud_medium', 'cloud_low', 'fog'
        ]
...
class HTTPWeatherClient(WeatherDataClient):
    ...
    def _setup_session(self):
        ...
        self.session.timeout = getattr(self.config, 'timeout', 30)
...
class WeatherDataProcessor:
    ...
    @staticmethod
    def standardize_dataframe(df: pd.DataFrame, source_info: DataSourceInfo) -> pd.DataFrame:
        ...
        variable_mappings = {
            # Temperature
            'temperature': ['air_temperature_2m', 'temperature_2m', 'temp', 't2m', 'temperature'],
            'pressure': ['air_pressure_at_sea_level', 'surface_air_pressure', 'msl', 'slp', 'pressure'],
            'humidity': ['relative_humidity_2m', 'relative_humidity', 'rh2m', 'humidity'],
    ...
    def _add_derived_variables(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if ('dew_point' not in df_derived.columns or df_derived['dew_point'].isna().all()) and \
           'temperature' in df_derived.columns and 'humidity' in df_derived.columns:
            import numpy as np
...
class DataClientRegistry:
    ...
    def register_client(self, name: str, client: WeatherDataClient, priority: int = 0):
        ...
        self.default_priority = sorted(
            self.clients.keys(),
            key=lambda x: self.clients[x]['priority'],
            reverse=True
        )
    ...
    def get_best_client(self, requirements: Optional[Dict[str, Any]] = None) -> Optional[WeatherDataClient]:
        ...
        for client_name in self.default_priority:
            client = self.clients[client_name]['client']
