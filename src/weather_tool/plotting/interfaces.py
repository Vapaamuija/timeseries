...
class PlotStyle(Enum):
    ...
    METEOGRAM = "meteogram"  # Meteogram diagram
...
class PlotConfig:
    ...
    # T-series specific
    tseries_height_ratios: Optional[List[float]] = None
    ...
    def get_effective_colors(self) -> Dict[str, str]:
        ...
        if self.colors:
            ...
            colors.update(
                {"background": "#ffffff", "text": "#333333", "grid": self.grid_color}
            )
    ...
    def get_effective_time_axis_padding(self) -> Optional[Dict[str, Any]]:
        ...
        if self.time_axis_padding:
            return {
                ...
                "start_padding_hours": self.time_axis_padding.get(
                    "start_padding_hours"
                ),
...
class WeatherPlotter(ABC):
    ...
    @abstractmethod
    def create_plot(
        self, data: pd.DataFrame, airport: Dict[str, Any], **kwargs: Any
    ) -> Figure:
...
class PlotterRegistry:
    ...
    def __init__(self) -> None:
        ...
        self.symbol_renderers: Dict[str, Dict[str, Any]] = {}

    def register_plotter(
        self, name: str, plotter_class: type, is_default: bool = False
    ) -> None:
        ...
        """
        self.plotters[name] = {"class": plotter_class, "is_default": is_default}

    def register_symbol_renderer(
        self, name: str, renderer_class: type, is_default: bool = False
    ) -> None:
    ...
    def get_plotter(self, name: str, config: PlotConfig) -> Optional[WeatherPlotter]:
        ...
        if plotter_info:
            return plotter_info["class"](config)  # type: ignore
        return None

    def get_symbol_renderer(
        self, name: str, config: PlotConfig
    ) -> Optional[WeatherSymbolRenderer]:
        ...
        if renderer_info:
            return renderer_info["class"](config)  # type: ignore
        return None

    def get_default_plotter(
        self, style: PlotStyle, config: PlotConfig
    ) -> Optional[WeatherPlotter]:
        ...
        for name, info in self.plotters.items():
            if info["is_default"] and style.value in name.lower():
                return info["class"](config)  # type: ignore

        # Fallback to first available plotter
        if self.plotters:
            first_plotter = next(iter(self.plotters.values()))
            return first_plotter["class"](config)  # type: ignore
    ...
    def list_plotters(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            name: {"is_default": info["is_default"]}
            for name, info in self.plotters.items()
        }
    ...
    def list_symbol_renderers(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {"is_default": info["is_default"]}
            for name, info in self.symbol_renderers.items()
        }
