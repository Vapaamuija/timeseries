"""
Reusable plotting components for Norwegian meteogram visualization.

This module provides the essential components needed for the Norwegian meteogram plotter.
"""
...
class WindBarbPlotter:
    ...
    def plot_barbs(self, ax: Axes, data: pd.DataFrame, max_barbs: int = 15, 
                   ylabel: str = None, length: int = 7) -> None:
        ...
        # Plot wind barbs
        ax.barbs(x_positions, y_positions, u, v, length=length, 
                barbcolor='black', flagcolor='black', linewidth=1.2, fill_empty=True)
...
class LayoutManager:
    """Manages layout and positioning for Norwegian meteogram."""
    
    def __init__(self, config: PlotConfig):
        """Initialize layout manager."""
        self.config = config
    
    def create_meteogram_layout(self, panel_heights, figsize: Tuple[int, int] = (16, 12), 
                               panel_padding=None, layout_config=None) -> Tuple[Figure, List[Axes]]:
        """Create the Norwegian meteogram layout with proper panel organization.
...
class CloudLayerPlotter:
    """Specialized plotter for cloud layers with proper Norwegian meteogram styling."""
    
    def __init__(self, config: PlotConfig):
        """Initialize cloud layer plotter."""
        self.config = config
        self.axis_positions = {}  # Store axis positions for legend placement
    
    def plot_cloud_layers(self, axes: List[Axes], data: pd.DataFrame, 
                         variables: List[str], labels: List[str], 
                         grid_interval: int = 1, label_padding=None) -> None:
        ...
        for ax, variable, label in zip(axes, variables, labels):
            ...
            self._plot_single_layer_no_legend(ax, data, variable, grid_interval, padding_config)
        
        # Then add all legends after plotting is complete
        self._add_all_legends(labels, label_padding)
    
    def _plot_single_layer_no_legend(self, ax: Axes, data: pd.DataFrame, 
                                     variable: str, grid_interval: int, padding_config: dict = None) -> None:
        ...
        if variable in data.columns:
            ...
            cloud_data_numeric = pd.to_numeric(data[variable], errors='coerce').fillna(0)
            ...
            cloud_data_percent = np.clip(cloud_data_percent, 0, 100)
            ...
            ax.fill_between(time_indices, lower_bound, upper_bound,
                           color=colors['color'], alpha=colors['alpha'], 
                           edgecolor=colors['edge_color'], linewidth=colors['edge_width'],
                           step='mid')
            
            # Add outline for better definition
            ax.plot(time_indices, upper_bound, color=colors['edge_color'], 
                   linewidth=colors['edge_width'], alpha=0.8)
            ax.plot(time_indices, lower_bound, color=colors['edge_color'], 
                   linewidth=colors['edge_width'], alpha=0.8)
        
        # Format axis to match Norwegian meteogram style
        ax.set_ylim(0, 1)
        if padding_config:
    ...
    def _get_cloud_colors(self, variable: str) -> dict:
        ...
        default_colors = {
            'cloud_high': {'color': '#87CEEB', 'alpha': 1.0, 'edge_color': '#5F9EA0', 'edge_width': 0.5},
            'cloud_medium': {'color': '#4682B4', 'alpha': 1.0, 'edge_color': '#2F4F4F', 'edge_width': 0.5},
            'cloud_low': {'color': '#2F4F4F', 'alpha': 1.0, 'edge_color': '#1C1C1C', 'edge_width': 0.5},
            'fog': {'color': '#48CAE4', 'alpha': 1.0, 'edge_color': '#0077BE', 'edge_width': 0.5}
        }
        ...
        try:
            if hasattr(self.config, 'variable_config') and self.config.variable_config:
                ...
                if var_config:  # Only use config if it exists and is not empty
                    logger.debug("Using configuration colors for %s: %s", variable, var_config)
                    return {
                        'color': var_config.get('color', default_colors.get(variable, default_colors['cloud_low'])['color']),
                        'alpha': var_config.get('alpha', default_colors.get(variable, default_colors['cloud_low'])['alpha']),
                        'edge_color': var_config.get('edge_color', default_colors.get(variable, default_colors['cloud_low'])['edge_color']),
                        'edge_width': var_config.get('edge_width', default_colors.get(variable, default_colors['cloud_low'])['edge_width'])
                    }
                else:
                    logger.debug("No configuration found for %s, using defaults", variable)
            else:
                logger.debug("No variable_config available, using defaults for %s", variable)
    ...
    def _add_all_legends(self, labels: List[str], label_padding=None) -> None:
        ...
        if label_padding:
            ...
            if isinstance(label_padding, dict):
                ...
                cloud_label_offset = label_padding.get('cloud_label_offset', cloud_label_offset)
            else:
                cloud_label_x = getattr(label_padding, 'cloud_label_x', cloud_label_x)
                cloud_label_offset = getattr(label_padding, 'cloud_label_offset', cloud_label_offset)
        ...
        for label in labels:
            if label in self.axis_positions:
                ...
                fig.text(cloud_label_x, legend_y, label, 
                        fontsize=9, va='center', ha='right',
                        color='#808080', fontweight='normal',
                        transform=fig.transFigure)
                
                # Add colored square with configurable offset
                square_x = cloud_label_x + cloud_label_offset
                colored_rect = plt.Rectangle(
                    (square_x, legend_y - 0.003), 0.008, 0.006,
                    facecolor=colors['color'], alpha=colors['alpha'],
                    edgecolor=colors['edge_color'], linewidth=colors['edge_width'],
    ...
    def _add_colored_legend(self, ax: Axes, label: str, colors: dict) -> None:
        ...
        colored_rect = plt.Rectangle(
            (0.13, legend_y - 0.003), 0.008, 0.006,
            facecolor=colors['color'], alpha=colors['alpha'],
            edgecolor=colors['edge_color'], linewidth=colors['edge_width'],
            transform=fig.transFigure, clip_on=False
        )
...
class FormattingUtils:
    ...
    @staticmethod
    def format_time_axis(ax: Axes, data: pd.DataFrame, grid_interval: int = 1, padding_config: dict = None) -> None:
        """Format time axis with proper Norwegian meteogram styling.
    ...
    @staticmethod
    def add_time_labels(ax: Axes, data: pd.DataFrame, time_interval: int = 1, padding_config: dict = None) -> None:
    ...
    @staticmethod
    def add_vertical_grid(target_axis: Axes, total_time_points: int, grid_line_interval: int = 1, grid_line_color: str = 'grey', grid_transparency: float = 0.3, day_boundary_indices: list = None) -> None:
        ...
        for time_point_index in range(0, total_time_points, grid_line_interval):
            ...
            target_axis.axvline(x=time_point_index, color=grid_line_color, linestyle='-', alpha=grid_transparency, linewidth=grid_line_width)
    
    @staticmethod
    def add_horizontal_grid(ax: Axes, color: str = 'lightgray', alpha: float = 0.4, linestyle: str = ':', linewidth: float = 0.5, grid_values: list = None) -> None:
        ...
        if grid_values:
            ...
            for y_val in grid_values:
                ax.axhline(y=y_val, color=color, linestyle=linestyle, alpha=alpha, linewidth=linewidth)
        else:
    ...
    @staticmethod
    def calculate_nice_grid_values(y_axis_minimum: float, y_axis_maximum: float, maximum_grid_lines: int = 6) -> list:
        ...
        grid_start_value = math.floor(y_axis_minimum / final_step_size) * final_step_size
    ...
    def add_date_labels(ax: Axes, data: pd.DataFrame) -> None:
        ...
        for pos, date in zip(date_positions, unique_dates):
            ax.text(pos, -0.15, date.strftime('%Y-%m-%d'), 
                   ha='left', va='top', fontsize=8, 
                   transform=ax.get_xaxis_transform())
    
    @staticmethod
    def apply_norwegian_styling(ax: Axes, ylabel: str = None) -> None:
        """Apply Norwegian meteogram styling to an axis.
...
class BottomLegendPlotter:
    ...
    def add_bottom_legend(self, meteogram_figure: Figure, weather_data: pd.DataFrame, legend_positioning_config=None) -> None:
        ...
        legend_vertical_position = 0.02  # Default bottom position
        legend_horizontal_start_position = 0.15  # Default left start position
        legend_item_horizontal_spacing = 0.12  # Default horizontal spacing between legend items
        
        if legend_positioning_config:
            # Handle both dictionary and object-style label padding
            if isinstance(legend_positioning_config, dict):
                legend_vertical_position = legend_positioning_config.get('bottom_legend_y', legend_vertical_position)
                legend_horizontal_start_position = legend_positioning_config.get('bottom_legend_x_start', legend_horizontal_start_position)
                legend_item_horizontal_spacing = legend_positioning_config.get('bottom_legend_spacing', legend_item_horizontal_spacing)
            else:
                legend_vertical_position = getattr(legend_positioning_config, 'bottom_legend_y', legend_vertical_position)
                legend_horizontal_start_position = getattr(legend_positioning_config, 'bottom_legend_x_start', legend_horizontal_start_position)
                legend_item_horizontal_spacing = getattr(legend_positioning_config, 'bottom_legend_spacing', legend_item_horizontal_spacing)
        ...
        legend_text_font_weight = font_configuration.get('weights', {}).get('annotation', 'normal')
        ...
        if 'temperature' in weather_data.columns:
            weather_legend_items.extend([
                {
                    'color': configured_color_scheme.get('temperature_above', '#d62728'),
                    'label': 'Temperature ≥0°C',
                    'type': 'line'
                },
                {
                    'color': configured_color_scheme.get('temperature_below', '#1f77b4'),
                    'label': 'Temperature <0°C',
                    'type': 'line'
                }
            ])
        ...
        cloud_layer_display_labels = ['High Clouds', 'Medium Clouds', 'Low Clouds', 'Fog']
        
        for cloud_variable, display_label in zip(available_cloud_variables, cloud_layer_display_labels):
            if cloud_variable in weather_data.columns:
                # Get cloud colors from variable config
                cloud_layer_color_config = self._get_cloud_colors_for_legend(cloud_variable)
                weather_legend_items.append({
        ...
        for legend_item in weather_legend_items:
            ...
            self._draw_legend_item(meteogram_figure, current_legend_x_position, legend_vertical_position, legend_item, 
                                 legend_text_font_size, legend_text_font_weight, legend_text_color)
    ...
    def _draw_legend_item(self, meteogram_figure: Figure, legend_item_x_position: float, legend_item_y_position: float, legend_item_config: dict, 
                         text_font_size: int, text_font_weight: str, text_display_color: str) -> None:
        ...
        if legend_item_config['type'] == 'line':
            ...
            meteogram_figure.add_artist(plt.Line2D([legend_item_x_position, legend_item_x_position + legend_symbol_width], [symbol_center_y, symbol_center_y],
                                    color=legend_item_config['color'], linewidth=2, 
                                    transform=meteogram_figure.transFigure, clip_on=False))
        
        elif legend_item_config['type'] == 'dashed_line':
            # Draw a dashed line
            symbol_center_y = legend_item_y_position + legend_symbol_height / 2
            meteogram_figure.add_artist(plt.Line2D([legend_item_x_position, legend_item_x_position + legend_symbol_width], [symbol_center_y, symbol_center_y],
                                    color=legend_item_config['color'], linewidth=2, linestyle='--',
                                    transform=meteogram_figure.transFigure, clip_on=False))
        
        elif legend_item_config['type'] == 'bar':
            # Draw a small rectangle representing a bar
            bar_symbol_rectangle = plt.Rectangle((legend_item_x_position, legend_item_y_position), legend_symbol_width, legend_symbol_height,
                               facecolor=legend_item_config['color'], alpha=0.7,
                               transform=meteogram_figure.transFigure, clip_on=False)
            ...
            filled_area_symbol_rectangle = plt.Rectangle((legend_item_x_position, legend_item_y_position), legend_symbol_width, legend_symbol_height,
                               facecolor=legend_item_config['color'], alpha=1.0,
                               transform=meteogram_figure.transFigure, clip_on=False)
            meteogram_figure.patches.append(filled_area_symbol_rectangle)
        
        # Add text label
        legend_text_x_position = legend_item_x_position + legend_symbol_width + symbol_text_spacing
        meteogram_figure.text(legend_text_x_position, legend_item_y_position + legend_symbol_height / 2, legend_item_config['label'],
                fontsize=text_font_size, fontweight=text_font_weight, color=text_display_color,
                va='center', ha='left', transform=meteogram_figure.transFigure)


class GridManager:
    """Manages grid lines across all panels in the Norwegian meteogram."""
    
    def __init__(self, config: PlotConfig):
        """Initialize grid manager."""
        self.config = config
    
    def add_unified_grid(self, meteogram_axes_dict: Dict[str, Axes], time_series_data: pd.DataFrame, 
                        hourly_grid_interval: int = 1, axis_padding_config: dict = None) -> None:
        ...
        vertical_grid_line_positions = list(range(0, len(time_series_data), hourly_grid_interval))
        
        # Calculate x-limits with padding
        if axis_padding_config:
            x_axis_start_limit, x_axis_end_limit = FormattingUtils.calculate_padded_xlim(time_series_data, axis_padding_config)
        else:
        ...
        for axis_identifier, panel_axis in meteogram_axes_dict.items():
            if axis_identifier not in ['time_axis']:  # Skip time axis as it handles its own grid
                for grid_line_position in vertical_grid_line_positions:
                    panel_axis.axvline(x=grid_line_position, color='grey', linestyle='-', alpha=0.3, linewidth=0.5)
