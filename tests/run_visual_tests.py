#!/usr/bin/env python3
"""
Helper script to run and debug Norwegian meteogram visual tests.
...
def run_visual_tests(test_name=None, verbose=False, show_images=False):
    ...
    if test_name:
        cmd.append(f"tests/test_norwegian_meteogram_visual.py::{test_name}")
    else:
        cmd.append("tests/test_norwegian_meteogram_visual.py")
...
def compare_images():
    ...
    try:
        ...
        if ref_img.size != gen_img.size:
            print("⚠️  Images have different sizes. Resizing generated to match reference.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run and debug Norwegian meteogram visual tests",
...
Examples:
  ...
  python run_visual_tests.py --test TestNorwegianMeteogramVisual::test_norwegian_meteogram_matches_reference
        """
    )
    
    parser.add_argument(
        "--test", "-t",
        help="Specific test to run (e.g., TestNorwegianMeteogramVisual::test_norwegian_meteogram_matches_reference)"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
